    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  else if (qp_solver_type_ == "unconstraint_fast")//default
  {
...
//note-tianyu main mpc车辆控制主函数
void MPCFollower::timerCallback(const ros::TimerEvent &te)
{

  /* guard */
  //note-tianyu 不满足相应条件的话直接按着制动处理
...
{
  const int N = mpc_param_.prediction_horizon;//得到预测范围的步长
  const double DT = mpc_param_.prediction_sampling_time; //得到周期
  //依次得到状态量（3）、控制量（1）、输出量的维度（2）
  ...
  if (vehicle_model_type_ == "kinematics")//default，给状态量赋予初值
  {
...
void MPCFollower::callbackRefPath(const autoware_msgs::Lane::ConstPtr &msg)
{
  ...
  //note-tianyu 计算相对时间，设置路径起点的时间为0，之后根据每个路径点的目标速度以及点与点之间的距离计算出车辆到达每个路径点时相对于起点的时间
  ...
  traj.vx.back() = end_velocity; // also for end point //设置最后一个点的目标速度为0
