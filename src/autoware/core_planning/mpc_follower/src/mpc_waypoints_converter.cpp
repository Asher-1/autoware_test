...
public:
  ...
  {
    //note-tianyu 发布局部路径信息（只包含最近路径点前后的若干个点）
...
private:
  ...
  void callbackFinalWaypoints(const autoware_msgs::Lane &final_waypoints)
  {
    ...
    //将最近点之前的back_waypoints_num_个路径点注入mpc_waypoints中，不包括最近点
    ...
    //将最近点之后的front_waypoints_num_个路径点加入mpc_waypoints中，不包括最近点
    for (int i = 3; i < final_end; ++i)
    {
