...
AstarSearch::AstarSearch()
{
  ...
  // base configs
  private_nh_.param<bool>("use_back", use_back_, true);// 是否朝后规划flag
  ...
  private_nh_.param<double>("minimum_turning_radius", minimum_turning_radius_, 6.0);//最小转弯半径和转角分辨率配合求取step

  // search configs
  private_nh_.param<int>("theta_size", theta_size_, 48);//转角分辨率
  private_nh_.param<double>("angle_goal_range", angle_goal_range_, 6.0);// 注释位置—check是否达到goal的yaw阀值
  private_nh_.param<double>("curve_weight", curve_weight_, 1.2);
  private_nh_.param<double>("reverse_weight", reverse_weight_, 2.00);
  // note-tianyu check是否是goal的位置阀值
...
// note-tianyu 将转换到costmap坐标系下的pose，转换到costmap的索引
...
bool AstarSearch::search()
{
 ...
 // openlist为空，搜索失败
  while (!openlist_.empty())
  {
    ...
    // Expand nodes
    // note-tianyu 遍历current node的相邻nodes
...
//note-tianyu 对当前simple_node在costmap上面进行碰撞检测
