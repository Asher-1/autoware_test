...
void LaneSelectNode::processing()
{
  ...
  //note-tianyu 只有当找到了非-1的closed waypoint，才能向下执行着
  findNeighborLanes();
  ...
  {
    updateChangeFlag();
    //note-tianyu 根据change_flag提前创建好左右准备变道的lane
    createLaneForChange();

    publishLane(std::get<0>(tuple_vec_.at(current_lane_idx_)));
    publishClosestWaypoint(std::get<1>(tuple_vec_.at(current_lane_idx_)));
    publishChangeFlag(std::get<2>(tuple_vec_.at(current_lane_idx_)));
    //note-tianyu 发布vehicle在lane中的位置
    publishVehicleLocation(std::get<1>(tuple_vec_.at(current_lane_idx_)), lane_array_id_);
  }
  publishVisualizer();
  resetSubscriptionFlag();
...
// note-tianyu 根据当前pose找到距离最近的waypoint
                                 ...
                                 const double distance_threshold, const int search_closest_waypoint_minimum_dt)
{
  ...
  // note-tianyu 如果没有找到上一个closed waypoint的id，则从当前pose的前面的waypoint开始搜寻  
  ...
  {
    // note-tianyu current pose和上一个最邻近waypoint之间距离不能太远
  ...
  {
    // note-tianyu 将当前遍历waypoint转换到以current pose为原点的坐标系下
  ...
  std::vector<double> dist_vec;
  dist_vec.reserve(idx_vec.size());
  // note-tianyu 依次遍历候选waypoint，得到current pose同各个候选waypoint的距离
  for (const auto &el : idx_vec)
  {
