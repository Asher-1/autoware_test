...
autoware_msgs::Lane apply_crossroad_acceleration(const autoware_msgs::Lane& lane, double acceleration)
{
  ...
  for (size_t i = 0; i < l.waypoints.size(); ++i)
  {
    ...
    if (i == 0)
    {
      //note-tianyu-pl 判断lane是否符合cross road的情况,r小于10才被认为是
  ...
  if (start_indexes.empty() && end_indexes.empty())//没有满足条件，在这直接return了，不会进行后面的加速操作
    return l;
                                        ...
                                        double stopline_search_radius)
{
  ...
  //note-tianyu-pl 确定stopline是否已经出现lane经过的范围之内
...
{
  autoware_msgs::Lane l = lane;
  //note-tianyu 得到了stopline邻近的waypoints的index
...
void cache_lane(const vector_map::LaneArray& msg)
{
  ...
  std::cout<< "lanes received" <<std::endl;
  update_values();
}
...
void cache_node(const vector_map::NodeArray& msg)
{
  ...
  std::cout<< "nodes received" <<std::endl;
  update_values();
}
...
void cache_stopline(const vector_map::StopLineArray& msg)
{
  ...
  std::cout<< "stoplines received" <<std::endl;
  update_values();
}
...
void cache_dtlane(const vector_map::DTLaneArray& msg)
{
  ...
  std::cout<< "dtlanes received" <<std::endl;
  update_values();
}
