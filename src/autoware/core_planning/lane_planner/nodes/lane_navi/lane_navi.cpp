...
void create_waypoint(const tablet_socket_msgs::route_cmd& msg)
{
  ...
  //note-tianyu 通过route_cmd msg得到一个粗糙的vector map
  ...
  //统计fine_mostleft_vmap中所包含的lane个数
...
void LocalTrajectoryCostCallback(const autoware_msgs::Lane& msg)
{
  local_trajectory_best = msg;
}
