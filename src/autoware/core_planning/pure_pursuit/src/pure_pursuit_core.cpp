...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    double kappa = 0;
    // note-tianyu 判断能否获取曲率的标志位，是否能够获得曲率，作为后面速度输出的重要判据,同时获取曲率
    ...
    // note-tianyu 发布各种用于rviz显示的Marker
...
{
  if (velocity_source_ == enumToInteger(Mode::dialog))//note-tianyu-mo if(1)则手动给ahead距离
  {
...
{
  velocity_source_ = config->param_flag;//note-tianyu 该flag用于切换是按着恒定速度行驶还是按着waypoint上的速度行驶
  ...
  const autoware_msgs::LaneConstPtr& msg)
{
  command_linear_velocity_ =
    (!msg->waypoints.empty()) ? msg->waypoints.at(5).twist.twist.linear.x : 0; //note-tianyu-TODO
  //如果加入了虚拟的路径结束点执行此分支，相当于在原有基础上新加入了一定数量的点
  if (add_virtual_end_waypoints_)
  {
