...
// note-tianyu 主要目的是lookahead_distance_来对目标waypoint的位置来进行插值运算
  ...
  int next_waypoint, geometry_msgs::Point* next_target) const
{
  ...
  // note-tianyu 如果距离直线距离过大，失败，可以认为pose飘逸
  if (d > search_radius)
    return false;
...
bool PurePursuit::canGetCurvature(double* output_kappa)
{
  ...
  // 计算当前position和current_waypoints_中的每一个waypoint的平面距离，要保证存在满足minimum_lookahead_distance_的路标点
