...
bool PlanningHelpers::GetRelativeInfo(const std::vector<WayPoint>& trajectory, const WayPoint& p, RelativeInfo& info, const int& prevIndex )
{
  ...
  {
    //note-tianyu 在trajectory中快速得到和当前pose最近的下一个路标点的id
		//后面的p0和p1表示下一个路标点和下下一个路标点，注意他们的坐标貌似是相对于当前pose下的
    ...
    std::vector<WayPoint>& sampledPoints)
{
  ...
  double dummyd = 0;
	...
	//计算生车轨迹的索引个数限制
	//note-tianyu 参考图例来理解open_planner过程
  ...
  if(iLimitIndex >= originalCenter.size())//originalCenter为当前输入的lane sections
...
//  }
  ...
  //注释位置—插入第一部分路径，也就是carTip和rollIn部分
	//插入carTip部分
