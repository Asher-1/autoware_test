...
ContourTracker::ContourTracker()
{
  ...
  //note-tianyu 这里订阅从欧式聚类的模块处发布的聚类结果
...
void ContourTracker::callbackGetCloudClusters(const autoware_msgs::CloudClusterArrayConstPtr &msg)
{
  ...
  {
    //note-tianyu 将接受到的聚类类型转换成"PlannerHNS::DetectedObject"类型
    ...
    //note-tianyu 计算碰撞时间时候用到（TTC），默认时候不执行
...
void ContourTracker::ImportCloudClusters(const autoware_msgs::CloudClusterArrayConstPtr& msg, std::vector<PlannerHNS::DetectedObject>& originalClusters)
{
  ...
  {
    //note-tianyu 有一些几何特征在聚类算法的过程中已经实现了，直接读进来就可以
    ...
    if(!IsCar(obj, m_CurrentPos, m_Map)) continue; //排除掉跟车的情况
    ...
    //note-tianyu 根据当前cluster所包含的点云几何特征，来估计其多边形形状
...
//note-tianyu 用于将track info以可视化形式显示出来
void ContourTracker::VisualizeLocalTracking()
{
  ...
  for(unsigned int i = 0; i < m_ObstacleTracking.m_InterestRegions.size(); i++)
  {
    ...
    circle_mkrs.header.frame_id = "map";//note-tianyu 
    circle_mkrs.header.stamp = ros::Time();
...
//  cout << endl;
  ...
  m_OutPutResults.header.frame_id = "velodyne";
